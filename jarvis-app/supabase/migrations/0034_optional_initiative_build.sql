-- 0034_optional_initiative_build.sql
-- Initiatives no longer require a build (some initiatives aren't build-dependent).
-- Reassigning an initiative's build (including clearing it) cascades to every
-- epic under it and every ticket under those epics, so the hierarchy never
-- drifts out of sync with its parent initiative.

alter table initiatives alter column build_id drop not null;
alter table epics alter column build_id drop not null;
alter table tickets alter column build_id drop not null;

create or replace function public.reassign_initiative_build(p_initiative_id uuid, p_build_id uuid)
returns initiatives
language plpgsql
security definer
...
begin
  ...
  set build_id = p_build_id, updated_at = now()
  where epic_id in (select id from public.epics where initiative_id = p_initiative_id);

  return i;
end;
$function$;

-- advance_ticket: fix WIP-count queries that compared `build_id = t.build_id`,
-- which SQL's three-valued logic never treats as true when t.build_id is null
-- (now possible) — buildless tickets would silently never count toward any
-- WIP limit. Switched to `is not distinct from`. No other behavior changes.
...
begin
  ...
  if from_stage <> p_to_stage then
    ...
    if wip_limit is not null then
      select count(*) into wip_count from tickets where build_id is not distinct from t.build_id and stage = p_to_stage;
    ...
    loop
      if wg.scope = 'owner' then
        ...
        where build_id is not distinct from t.build_id and stage = any(wg.stage_keys) and assignee_id = actor_id;
      else
        select count(*) into wip_count from tickets
        where build_id is not distinct from t.build_id and stage = any(wg.stage_keys);
      end if;
