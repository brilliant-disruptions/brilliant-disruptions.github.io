-- Rename "kill gate" to "gate" in user-facing error text only. The stored
-- required_checklist_key values (e.g. kill_gate_backlog_progress) are left
-- untouched since they're an internal storage key, not a display string, and
-- renaming the prefix would orphan checklist state already saved on existing
-- tickets/epics/initiatives.
        ...
        if rule.required_checklist_key is not null and array_length(rule.checklist_items, 1) > 0 then
          ...
          foreach item in array rule.checklist_items loop
            if coalesce((checklist_state -> item)::text, 'false') <> 'true' then
              raise exception 'gate checklist item not completed: %', item;
            end if;
          end loop;
        end if;
        exit;
