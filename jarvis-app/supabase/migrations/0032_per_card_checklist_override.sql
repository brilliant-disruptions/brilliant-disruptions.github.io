-- ── Per-card checklist overrides: a card can add/remove kill-gate checklist
--    items without changing the shared board rule. The override list, when
--    present, is stored at custom_fields[required_checklist_key || '__items']
--    and fully replaces the rule's checklist_items for that card. Mirrors the
--    client-side effectiveChecklistItems() helper in lib/workflow-gating.ts.
...
declare
  ...
  checklist_items_override jsonb;
  ...
  if from_stage <> p_to_stage then
    ...
    loop
      ...
      if rule.to_stage = p_to_stage then
        ...
        if rule.required_checklist_key is not null then
          checklist_items_override := t.custom_fields -> (rule.required_checklist_key || '__items');
          if checklist_items_override is not null and jsonb_typeof(checklist_items_override) = 'array' then
            select array_agg(x) into checklist_items from jsonb_array_elements_text(checklist_items_override) x;
          else
            checklist_items := rule.checklist_items;
          end if;

          if checklist_items is not null and array_length(checklist_items, 1) > 0 then
